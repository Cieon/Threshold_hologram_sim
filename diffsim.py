...
sim_var = [16, 632.8, 30, 30, 0.1, 100, 80, 0.5, 1.0, 0, 0, 0]  # Default values of simulation parameters in order of sim_names
...
if menu is 1:
    ...
    if int(loop_var[0]) is 0:
        # SINGLE MODE
        # Create a MonochromaticField class instance
        ...
        # Use the phase_sampling method to write the hologram phase using threshold cells
        F.phase_sampling(Phase, w0=sim_var[6] / 1000 * mm, size=size, sampling=int(sim_var[0]), threshold=sim_var[7],
                         amp_mod=sim_var[8], steps=int(sim_var[9]), extended=int(sim_var[10]), verbose=bool(sim_var[11]))
        # Apply a spherical wave over the pattern to make it focus at the specific distance
        F.spherical_wave(z0=sim_var[5] * cm)
        # Integrate the field intensity to calculate diffraction efficiency
        F.integrate_intensity()

        # Propagate the pattern to the source of the spherical wave to reconstruct the image and acquire the plot rgb values
        rgb1 = F.compute_colors_at(sim_var[5] * cm)
        # Plot the acquired rgb values of the field
        F.plot(rgb1, xlim=[-F.extent_x / 2 * 1000, F.extent_x / 2 * 1000],
               ylim=[-F.extent_y / 2 * 1000, F.extent_y / 2 * 1000])
        # Conduct the data analysis of the produced field
        data = F.data_analysis(size, 1, export=True)
    elif int(loop_var[0]) is 1:
        # LOOP MODE

        # Date and time for directory and file naming
        ...
        # Open up the statistics file and write to it
        ...
        while (x <= loop_var[3]):
            ...
            rgb1 = F.compute_colors_at(sim_var[5] * cm)
            # Generate a path to the image with a specific name
            plotpath = './{}/{}={:.3f}_{}.png'.format(date, sim_names[loop_mapping[int(loop_var[1]) - 1]], x, time)
            F.plot(rgb1, xlim=[-F.extent_x / 2 * 1000, F.extent_x / 2 * 1000],
